...
class DataGenerator(tf.keras.utils.Sequence):
    ...
    def data_generation(self, file_name_list_temp):
        ...
        annotation_batch = np.empty((self.batch_size, self.num_coordinates))
        # Generate data
        for i, file_name in enumerate(file_name_list_temp):
            ...
            annotation_batch[i] = np.array(np.array(annotation['landmark']).reshape(-1, self.num_coordinates))
...
if __name__ == '__main__':
    train_data_generator = DataGenerator(image_dir=os.path.join('..', 'data', 'image'),
